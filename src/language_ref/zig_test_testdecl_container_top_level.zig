...
//--------------------------------------------------------------------------------------------------
// When the zig test tool is building a test runner, only resolved test declarations are included in the build.
...
// The code sample below uses the std.testing.refAllDecls(@This()) function call to reference all of the
// containers that are in the file including the imported Zig source file. The code sample also shows an
// alternative way to reference containers using the _ = C; syntax. This syntax tells the compiler to
// ignore the result of the expression on the right side of the assignment operator.
//--------------------------------------------------------------------------------------------------
